...
//    printf("====RT() ===== %lf \n",RT());
//    printf("====ideal enthalpy_mole ===== %lf \n",h_ideal);
   double h_nonideal = hresid();
//    printf("====non-ideal enthalpy_mole ===== %lf \n",h_nonideal);
   return h_ideal + h_nonideal;
//    printf("====enthalpy_mole ===== %lf \n",h_ideal + h_nonideal);
}
        ...
        int phase, doublereal rhoguess)
{
    ...
    for (int n = 0; n < 200; n++) {
        ...
        if (fabs(delMV/molarVolBase) < 1.0E-12) {
            conv = true;
            break;
        }
                                  ...
                                  double bn, double cn, double dn, double tc, double vc) const
{
    ...
    //     if (disc > 1e-10) {
    //         writelog("MixtureFugacityTP::solveCubic",
    ...
    if (disc > 0) {
        ...
        Vroot[1] = 0.0;
        Vroot[2] = 0.0;
        // printf("===the value of the single solution alpha = %lf , xN = %lf, sgn1  = %lf, p1  = %lf, sgn2  = %lf, p2 = %lf, tmp1 = %lf, tmp2 = %lf,\n",alpha,xN , sgn1 , p1 , sgn2, p2,tmp1, tmp2);
        ...
        double Z = min(min(beta, gamma), alpha);
        if (Z < 0) {
            Z = max(max(beta, gamma), alpha);
        }
        // printf("Sanity = %e\n ", Z);
        Vroot[0] = Z;
        Vroot[1] = Z;
        ...
        if (yN < 1e-18 && h < 1e-18) {
            ...
            if (yN > 0.0) {
                ...
                double Z = min(Vroot[0], Vroot[1]);
                if (Z < 0) {
                Z = max(Vroot[0], Vroot[1]);
                }
                // printf("Sanity = %e\n ", Z);
                Vroot[0] = Z;
                Vroot[1] = Z;
                Vroot[2] = Z;   
                if (Z < 0.0){
                    printf("liquid phase root is negative");
                }
                // printf("liquid xn and delta value = %lf, %lf", xN, delta);
            } else {
                tmp = pow(yN/(2*an), 1./3.);
                // In this case, tmp and delta must be equal.
                // if (fabs(tmp - delta) > 1.0E-9) {
                ...
                Vroot[2] = Z;        
                if (Z < 0.0){
                    printf("gas phase root is negative");
                }                
                // printf("gas xn and delta value = %lf, %lf, roots are Vroot[0] = %lf,Vroot[1] = %lf,Vroot[2] = %lf", xN, delta,Vroot[0],Vroot[1],Vroot[2]);
            }
        }
    }
    ...
    for (int i = 0; i < nSolnValues; i++) {
        for (int n = 0; n < 20; n++) {
            ...
            if (fabs(del) / (fabs(Vroot[i]) + fabs(del)) < 1.0E-12) {
                break;
            }
        ...
        if ((fabs(res) > 1.0E-12) && (fabs(res) > 1.0E-12 * fabs(dresdV) * fabs(Vroot[i]))) {
            writelog("MixtureFugacityTP::solveCubic(T = {}, p = {}): "
                "WARNING root didn't converge V = {}, an = {}, bn = {},"
                "cn = {},dn = {}, res = {}, i = {}, nSolnValues = {} ", T, pres, Vroot[i],an, bn,cn, dn, fabs(res),i,nSolnValues);
            writelogendl();
        }
    }
