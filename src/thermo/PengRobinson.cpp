...
void PengRobinson::setSpeciesCoeffs(const std::string& species, double a, double b,
                                    double w)
{
    printf("==============Using modified Peng-Robinson=================\n");
...
double PengRobinson::liquidVolEst(double T, double& presGuess) const
{
    ...
    calculateAB(atmp, btmp, aAlphatmp,temp);
...
void PengRobinson::updateMixingExpressions()
{
    ...
    calculateAB(m_a,m_b,m_aAlpha_mix,temp);
}

void PengRobinson::calculateAB(double& aCalc, double& bCalc, double& aAlphaCalc, double temp) const
{
    bCalc = 0.0;
    aCalc = 0.0;
    aAlphaCalc = 0.0;
    ...
    for (size_t i = 0; i < m_kk; i++) {
        ...
        for (size_t j = 0; j < m_kk; j++) {
            ...
            coeff1 += moleFractions_[i] * moleFractions_[j] * sqrt(m_a_coeffs(i, i)*m_a_coeffs(j, j))*(1+m_kappa[i])*(1+m_kappa[j]);
            coeff2 += moleFractions_[i] * moleFractions_[j] * sqrt(m_a_coeffs(i, i)*m_a_coeffs(j, j))*(m_kappa[i]*(1+m_kappa[j])/sqrt(speciesCritTemperature(m_a_coeffs(i,i),m_b_coeffs[i])) + m_kappa[j]*(1+m_kappa[i])/sqrt(speciesCritTemperature(m_a_coeffs(j,j),m_b_coeffs[j])));
            coeff3 += moleFractions_[i] * moleFractions_[j] * sqrt(m_a_coeffs(i, i)*m_a_coeffs(j, j))*(m_kappa[i]*m_kappa[j]/sqrt(speciesCritTemperature(m_a_coeffs(i,i),m_b_coeffs[i])*speciesCritTemperature(m_a_coeffs(j,j),m_b_coeffs[j])));
        }  
    }
    aAlphaCalc = coeff1 - coeff2 * sqrt(temp) + coeff3*temp;
}

double PengRobinson::daAlpha_dT() const
{
    double daAlphadT=0, coeff2 =0, coeff3 = 0.0;
    // for (size_t i = 0; i < m_kk; i++) {
    ...
    daAlphadT = - coeff2/(2*sqrt(temp)) + coeff3;
...
double PengRobinson::d2aAlpha_dT2() const
{
    ...
    d2aAlphadT2 = coeff2/(4*sqrt(temp)*temp);
                             ...
                             double Vroot[3]) const
{
    ...
    // doublereal Amat = aAlpha * pres / pow(GasConstant * T, 2);
    // doublereal Bmat = b * pres / (GasConstant * T);
    ...
    // p = (3.0 * coef_v1 - pow(coef_v2, 2)) / 3.0;
    // q = coef_v0 + 2 * pow(coef_v2, 3) / 27.0 - coef_v2 * coef_v1 / 3.0;
    ...
    //     Z1 = -coef_v2 / 3.0 + 2.0 * sqrt(fabs(p) / 3.0) * cos((phi) / 3.0);
    //     Z2 = -coef_v2 / 3.0 - 2.0 * sqrt(fabs(p) / 3.0) * cos((phi - Pi) / 3.0);
    //     Z3 = -coef_v2 / 3.0 - 2.0 * sqrt(fabs(p) / 3.0) * cos((phi + Pi) / 3.0);

    //     //jph For fully consistent results we should impose a fugacity constraint
    //     Z = std::min(std::min(Z1, Z2), Z3);
    //     if (Z < 0) {
    //         Z = std::max(std::max(Z1, Z2), Z3);
    //     }
    // }

    // Vroot[0] = GasConstant * T * Z;
    ...
    // return nSolnValues ;
    // double sanity = pow(Z,3)+pow(Z,2)*coef_v2+Z*coef_v1+coef_v0;
    //  printf("Sanity = %e\n ", sanity);
    // printf("======= solve cubic parameters (T, P)========%lf, %lf", T, pres);
